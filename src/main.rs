...
fn main() {
    ...
    objects.push(
        Box::new(
            Sphere::new(
                ...
                100.0,
                Box::new(Lambertian::new(Vec3::new(0.6, 0.6, 0.6)))
                )
            )
        );
    ...
    let img : image::RgbImage = image::ImageBuffer::from_fn(imgx as u32, imgy as u32, |x, y| {
        ...
        image::Rgb([col_screen.x as u8, col_screen.y as u8, col_screen.z as u8])
    });
    ...
    println!("Total Rays {}", (rays as f64));
    println!("Speed {} Rays per second", (rays as f64) / duration);
